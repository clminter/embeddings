...
# Set up Streamlit layout using HTML
st.title("⏭️ Semantic Search Agent")
st.write("A MINTER 🤖")
st.markdown("<h2>Document Control: Change Highlighter</h2>", unsafe_allow_html=True)
...
if documents:
    ...
    # Display search input box using HTML
    search_query = st.text_input("Enter your search query")

    # Control tree depth and chunk size using HTML
                ...
                if doc_index < len(document_names):
                    ...
                    for diff in diffs:
                        if diff[0] == -1:  # Deleted text
                            highlighted_text += f"<span style='background-color: #FFCCCC'>{diff[1]}</span>"
                            summary_text += f"[Deleted]: {diff[1]}\n"
                        elif diff[0] == 1:  # Inserted text
                            highlighted_text += f"<span style='background-color: #CCFFCC'>{diff[1]}</span>"
                            summary_text += f"[Inserted]: {diff[1]}\n"

                    if highlighted_text:
                        st.write("<h4>Highlighted Changes:</h4>", unsafe_allow_html=True)
